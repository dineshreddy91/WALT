...
from mmdet_old.apis import inference_detector, init_detector, show_result_pyplot
from mmdet_old.core.mask.utils import encode_mask_results
import numpy as np
import mmcv
import torch 
...
def main():
    #detect = detections('configs_local/swin/occ_mask_rcnn_swin_tiny_patch4_window7_mstrain_480-800_adamw_1x_parking_real_coco.py', 'cuda:0', model_path='work_dirs/occ_mask_rcnn_swin_tiny_patch4_window7_mstrain_480-800_adamw_1x_parking_real_coco/latest.pth')
    detect = detections('configs_local/swin/mask_rcnn_swin_tiny_patch4_window7_mstrain_480-800_adamw_1x_coco.py', 'cuda:0', model_path='mask_rcnn_swin_tiny_patch4_window7_1x.pth')
    ...
    import cv2, glob
    filenames = sorted(glob.glob('/code/demo/seq3/*'))
    count = 0
    for filename in filenames:
        ...
        count=count+1
        print(filename.replace('demo','demo/results_swin/'),'result_' + filename)
        import os
        #print(os.path.dirname(filename.replace('demo','demo/results_knis/')))

        try: 
            import os
            os.makedirs(os.path.dirname(filename.replace('demo','demo/results_swin/')))
            os.mkdirs(os.path.dirname(filename))
        except:
            print('done')
        cv2.imwrite(filename.replace('demo','demo/results_swin/'),img)
