...
def main():
    ...
    import cv2, glob
    filenames = sorted(glob.glob('/code/demo/seq8/*'))
    ...
    for filename in filenames:
        ...
        count=count+1
        print(filename.replace('demo','demo/results_knis/'),'result_' + filename)
        import os
        #print(os.path.dirname(filename.replace('demo','demo/results_knis/')))

        try: 
            import os
            os.makedirs(os.path.dirname(filename.replace('demo','demo/results_knis/')))
            os.mkdirs(os.path.dirname(filename))
        except:
            print('done')
        cv2.imwrite(filename.replace('demo','demo/results_knis/'),img)
